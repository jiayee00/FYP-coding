...
if __name__ == '__main__':
    ...
    affinity_nets = snf.make_affinity([omics_data_1.iloc[:, 1:].values.astype(float), 
                                       omics_data_2.iloc[:, 1:].values.astype(float), 
                                       omics_data_3.iloc[:, 1:].values.astype(float)],
                                      metric=args.metric, K=args.K, mu=args.mu)

    # Conduct similarity network fusion & produce fused similarity network in matrix form
